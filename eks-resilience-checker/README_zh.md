...
AI 驱动的 Agent Skill，对 Amazon EKS 集群执行自动化韧性评估，覆盖 **应用工作负载**（A1-A14）、**控制平面**（C1-C5）、**数据平面**（D1-D7）共 28 项最佳实践检查。输出结构化结果，可直接作为 `chaos-engineering-on-aws` Skill 的输入驱动混沌实验。
...
| 4 | **eks-resilience-checker** | 阶段 3: 评估与测试 | EKS 集群直连 | 28 项合规报告 + 实验推荐 |
...
│   └── prd.md                          # 产品需求文档
├── references/
│   ├── EKS-Resiliency-Checkpoints.md   # 28 项检查详细说明
